...
class Hittable_list():
    ...
    def hit_shadow(self, ray, t_min=0.001, t_max=10e8):
        ...
        # compute root of point to light source
        ...
        for index in ti.static(range(len(self.objects))):
            is_hit_tmp, root_tmp, hit_point_tmp, hit_point_normal_tmp, front_face_tmp, material_tmp, color_tmp =\
                self.objects[index].hit(ray, t_min, root_light_source)
            if is_hit_tmp:
                if material_tmp == 0:
...
class Camera:
    ...
    def get_ray(self, u, v):
        return Ray(
            ...
            self.cam_lower_left_corner[None] + u*self.cam_horizontal[None] + v*self.cam_vertical[None] - self.cam_origin[None]
            )